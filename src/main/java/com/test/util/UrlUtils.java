...
public class UrlUtils {
     ...
     * @Description: 
     * @author: wwb
     * @Date: 2018-10-09 17:34:40
     * @param: url
     * @return: java.lang.Integer
     */
    public static Integer getActivityIdByUrl(String url) {
        if (!StringUtils.isEmpty(url) && url.startsWith("/activity/")) {
            String[] split = url.split("/");
            if (split.length > 2) {
                String activityIdStr = split[2];
                if (!StringUtils.isEmpty(activityIdStr)) {
                    return Integer.valueOf(activityIdStr);
                }
            }
        }
        return null;
